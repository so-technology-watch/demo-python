...
class DriverService:
    def __init__(self, entity):
        self.dao = dao_commons.GenericDao(entity, Driver)

    def find_by_id(self, _id, _id2):
        """
        Tries to find an entity using its Id / Primary Key
        :param _id:
        :return: entity
        """
        entity = new_instance_with_double_primary_key(_id, _id2)
        query = build_query(entity)
        query2 = build_query_multiple_key(entity)
        return self.dao.do_select_multiple_keys(query, query2)
    ...
    def update(self, entity):
        ...
        query = build_query(entity)
        query2 = build_query_multiple_key(entity)
        result = self.dao.do_update(entity, query, query2)
        return result > 0
    ...
    def delete_by_id(self, _id, _id2):
        """
        ...
        query = build_query(entity)
        query2 = build_query_multiple_key(entity)
        result = self.dao.do_delete_multiple_key(query, query2)
        return result > 0
...
def new_instance_with_double_primary_key(_id, _id2):
    """
    build new entity with the given _id
    :param _id:
    :return: entity with the given _id
    """
    entity = Driver(_id, _id2, None, None)
    return entity


def build_query(entity):
    return Driver.person_id == '%s' % entity.person_id


def build_query_multiple_key(entity):
    return Driver.car_id == '%s' % entity.car_id
